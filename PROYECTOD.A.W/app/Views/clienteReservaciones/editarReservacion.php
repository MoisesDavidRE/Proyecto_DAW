...
<form action="<?= base_url('Cliente/editReservacion/' . $reservacion->idReservacion); ?>" method="POST" style="margin-bottom: -50px;">
      ...
      <table class="table table-bordered table-striped">
        ...
        <tbody>
          <tr>
            ...
            <td><input class="form-control" type="text"  value="<?php $query = "SELECT nombre FROM atraccion_animal WHERE id = $reservacion->atraccion_animal";
            $resultado = $db->query($query)->getResultArray();
            echo $resultado[0]["nombre"]?>">
          <input type="hidden" name="atraccion_animal" value="<?=$reservacion->atraccion_animal?>">  
          </td>
          </tr>
          <tr>
            <td>Reservación a nombre de: </td>
            <td><input class="form-control"  disabled type="text" value="<?php  $query = "SELECT nombre, apellido_Paterno, apellido_Materno FROM usuario WHERE numeroControl = $reservacion->usuario";
                                $resultado = $db->query($query)->getResultArray();
                                echo $resultado[0]["nombre"] . " " . $resultado[0]["apellido_Paterno"] . " " . $resultado[0]["apellido_Materno"]; ?>">
                                <input type="hidden" name="usuario" value="<?php $session = \Config\Services::session(); echo $session->get('idUsuario');?>"></td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td><input class="form-control" type="time" name="horaFin" value="<?= $reservacion->horaFin ?>"></td>
          </tr>
          <tr>
            <td>Estatus de la reservación</td>
            <td><input class="form-control"type="text" name="estatus" value="<?= $reservacion->estatus ?>"></td>
          </tr>
          <tr>
            <td>Monto total de la reservación</td>
            <td><input class="form-control"type="number" name="costoTotal" value="<?= $reservacion->costoTotal ?>"></td>
          </tr>
          <tr>
